...
export const RealocarModal: React.FC<RealocarModalProps> = ({ isOpen, alocacao, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        // Validações
        if (!alocacao || !alocacao.ferramenta_nome) {
          throw new Error('Nome da ferramenta não encontrado na alocação.');
        }
        ...
        const alocacaoPayload = {
          ...
          funcionario_nome: formData.funcionario_nome || '', // Valor padrão
          previsao_desalocacao: formData.previsao_desalocacao || null, // Valor padrão
        };
        ...
        console.log('✅ Nova alocação criada com sucesso');
        ...
        setError(err.message || 'Erro ao realocar. Tente novamente.');
      } finally {
        setLoading(false);
      }
